...
`index.html`이 이 발표자료의 편집 원본이다. 74장 구성, UOS 파란색 시각 체계,
...
4. 슬라이드는 74장을 유지하고, 작은 글씨를 늘리기보다 카드나 문장을 줄인다.
5. 영상은 한 슬라이드에 하나만 두고 `controls loop muted playsinline
   preload="metadata"`를 유지한다.

개념 영상은 28·29·30·34·35·43·45·48·59번에 있고, 실제 호버 영상은 66번에
...
현재 체크인된 PPTX는 HTML 기준본에서 재생성한 74장 산출물이다. 이후에도
사용자가 재생성을 요청할 때만 갱신한다.
