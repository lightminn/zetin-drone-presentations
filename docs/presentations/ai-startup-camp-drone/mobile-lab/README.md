...
5. 센서 경로는 중립 자세에서 `이 자세를 0°로 보정`한 뒤 Roll·Pitch와 AX·AY·AZ를 보고, 20초 호버링을 수행합니다. 터치 경로도 같은 축 관찰과 호버링을 끝까지 수행합니다.
...
- [ ] 센서 권한 거부·미지원·값 미수신 때 터치 조이스틱으로 IMU 관찰과 20초 도전을 완료했다.
