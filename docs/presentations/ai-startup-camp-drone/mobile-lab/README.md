...
4. 학생은 선택 표시 이름을 입력하거나 비워 두고, `센서로 체험 시작` 또는 `터치로 체험`을 고릅니다. 표시 이름에는 실명·연락처 등 개인정보를 입력하지 않으며, 점수를 제출하면 표시 이름과 점수가 발표자 화면에 공개될 수 있음을 먼저 안내합니다.
...
- Node 테스트: 권한 분기, 자세 보정·평활화, 조이스틱 벡터, 결정적 호버링·점수, 종료·재시작, 점수 제출 실패 뒤 로컬 결과 보존.
- Python 서버 테스트: 요청 검증, 중복 제출, 정적 파일, 50개 동시 제출 및 50개 중복 재시도.
