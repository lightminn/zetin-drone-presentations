...
없다. 따라서 아래 값은 실적 원가나 실제 기체의 확정 견적이 아니다. 부품 재고가
있다는 가정의 일정 시나리오와, 대체 부품을 포함해 가격이 확인되는 품목의 참고 합계이다.
화면에 쓰는 값의 기계 판독 원본은
...
| 10대 | 240~480시간 | 60~100인시 | 출력과 조립 병행 시 프린터 최소 10~20일 |

프린터 시간은 실측이나 슬라이서 결과가 아니다. 기체당 출력 질량을 0.4~0.8kg,
유효 출력량을 시간당 16.7g으로 둔 일정 가정이다. 따라서
`질량 ÷ 유효 출력량 = 24~48시간/대`이며, 10대는 단순히 10배로 확장했다.

직접 작업도 공정별 계획값을 합산했다.
...
| 프레임·전자부품 조립 | 2~3인시 |
| 배선·커넥터 작업 | 1~2인시 |
| 펌웨어 기록·교정·벤치 점검 | 2~3인시 |
| **합계** | **6~10인시** |

10대 직접 작업은 확인되지 않은 묶음 작업 할인을 넣지 않고 60~100인시로 단순
확장했다. 다음 조건을 전제로 한 일정 시나리오이다.

- 부품 재고 확보, 기구·회로·펌웨어 설계 완료
- 프린터 한 대에서 순차 출력
- 기체당 프린트 소재 0.4~0.8kg, 첫 출력 성공
- 조립된 FC PCB가 있는 상태에서 프레임·배선·교정 작업 시작
- 자유비행 튜닝, 반복 비행 검증, 재출력, 납기 대기는 제외

10~20일은 출력과 조립을 병행할 때의 프린터 점유 최소 기간이지 완성기 납기 약속이
아니다. 기체별 센서·모터 방향과 안전 점검은 생략하지 않는다. 프린터 두 대이면
출력 점유는 약 5~10일, 네 대이면 약 2.5~5일로 줄어들 수 있지만 사람 작업과
안전 점검은 같은 비율로 줄지 않는다.

## 가격이 확인되는 핵심 부품

아래 값은 실제 구매 SKU가 확정되지 않은 모터에는 대체 부품 참고가를 사용한다.
ESC는 현행 4S 배터리와의 호환을 명시한 공식 2~4S 제품 가격만 사용했다. 조회 기준은
2026-08-19이며 계획 환산값은 1 USD=1,425원, 1 AUD=1,001원이다. 해외 판매가는
배송·관부가세·환전 수수료를 제외했고, 판매처와 환율에 따라 바뀔 수 있다.

| 항목 | 1대 범위 | 산정 근거 |
|---|---:|---|
| 1750KV급 모터 4개 참고가 | 78,000~82,000원 | DYS SUNFUN 2207 1750KV 4개와 CCRC 2004 1750KV 4개의 판매가 범위. 실물 SKU·4S 추력 적합성 미확정 |
| EMAX BLHeli 30A ESC 4개 참고가 | 약 175,400원 | EMAX 공식 2~4S 제품가 30.78 USD×4. 4S 불가로 표시된 국내 저가 SKU는 제외 |
...
| **가격 확인 가능 참고 부품 합계** | **약 355,300~370,500원** | 완성기 총원가가 아니며 FC PCB·전원·배선·체결·배송·인건비 제외 |
| 선택 3901-L0X | **대당 +48,000~49,000원** | 국내 재고 참고가. 제조사 EOL 표기이므로 10대 조달 가능성 미확인 |

10대는 수량 할인이나 양산 효과를 가정하지 않고 같은 범위를 단순 확장해 약
3,553,000~3,705,000원으로 본다. 이는 완성기 총원가가 아니다. 다음 항목은 규격·수량
...
- DYS 모터: <https://dysrc.hk/product/>
- CCRC 1750KV급 모터 대체 판매가: <https://rcdrone.top/products/ccrc-sunhey-2004-brushless-motor>
- EMAX 30A ESC 공식 판매가: <https://shop.emaxmodel.com/collections/blheli-series-esc-1>
...
- 3901-L0X 제조사 EOL 표기: <https://www.mateksys.com/?portfolio=3901-l0x>

발표 화면에서는 판매처·환율·개별 단가를 나열하지 않는다. `예비 산정`, 가정,
미포함 항목을 함께 표시하고, 실제 제작 전에는 모터·ESC SKU, PCB 리비전,
슬라이서의 재료량·시간, PCB 실장 견적을 다시 확정한다.
