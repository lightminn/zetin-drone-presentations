...
3. CAD → 출력 → 조립 → 수정의 래피드 프로토타이핑
4. 비행 원리, 자세 추정과 Roll 믹서 오류 추적
5. 실제 펌웨어 SIL, 캐스케이드 제어와 Yaw 기준
6. 호버 추정·스로틀 조건 기반 안전 전환과 실기 테더 기록
7. 단기·중기·장기 기술 목표와 개발 결과
...
4장의 시간·비용은 실적이 아니라 계획 시나리오이다. 부품 보유, 조립 완료된
FC PCB, 프린터 한 대와 첫 출력 성공을 가정한다. 가격은 확인 가능한 참고 품목의
합계이며 완성기 총원가나 확정 BOM이 아니다. 상세 계산과 제외 범위는
