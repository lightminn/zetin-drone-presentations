...
공개본은 https://lightminn.github.io/zetin-drone-presentations/10min/ 에서 확인한다.

## 결과 흐름

1. 상용 제품과 자작 학습의 목적 구분
2. 직접 설계한 기체·보드와 조달해 통합한 비행·센서 부품
3. 전원 경로와 설계 → 출력 → 시험·파손 → 측정 → 개선의 래피드 프로토타이핑
4. 비행 원리, 자세 추정, 실제 축 디버깅과 별도 SIL 오류 검출
5. 실제 펌웨어 SIL, 캐스케이드 제어와 Yaw 기준
6. 호버 추정·스로틀 조건 기반 안전 전환과 착지 프로브 판정 한계
7. 테더 영상·모터 출력 해석과 제작·충전·검증 인프라, 지상 로봇 협업 계획
8. 감사 인사와 질의응답
...
열리면 음소거 상태로 자동 재생된다. 하단의 `전체화면` 버튼이나 `F` 키로
전체화면 발표를 시작하고, `Esc`로 종료한다. 전체화면에서는 썸네일과 하단
컨트롤이 숨겨지며 마우스를 움직일 때만 하단 컨트롤이 잠시 나타난다.
