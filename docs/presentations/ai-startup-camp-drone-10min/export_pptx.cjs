...
const DEFAULT_OUTPUT = path.join(DECK_DIR, "드론_10분_요약본.pptx");
...
function assertSafeRuntimeDir(runtimeDir) {
  ...
  if (!resolved.startsWith(tempRoot) || !path.basename(resolved).startsWith("drone-summary-pptx-export-")) {
...
function normalizeVideo(videoPath, runtimeDir, ffprobeBin, ffmpegBin) {
  ...
  run(ffmpegBin, [
    ...
    "-vf", "fps=30", "-r", "30", "-pix_fmt", "yuv420p",
...
async function buildPptx(records, runtimeDir, output, PptxGenJS, ffprobeBin, ffmpegBin) {
  ...
  pptx.subject = "자작 드론 비행제어 기술 요약";
  pptx.title = "자작 드론 비행 제어 시스템";
  pptx.lang = "ko-KR";
  pptx.theme = { headFontFace: "Noto Sans CJK KR", bodyFontFace: "Noto Sans CJK KR", lang: "ko-KR" };
  pptx.defineLayout({ name: "UOS_WIDE", width: PPTX_W, height: PPTX_H });
  pptx.layout = "UOS_WIDE";
...
async function main() {
  ...
  const runtimeDir = await fsp.mkdtemp(path.join(os.tmpdir(), "drone-summary-pptx-export-"));
  ...
  try {
    ...
    if (videoCount !== 1) throw new Error(`expected 1 video slide, found ${videoCount}`);
